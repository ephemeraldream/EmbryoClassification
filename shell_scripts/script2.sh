...
for ev2_file in *.ev2; do
    ...
    WHERE NOT EXISTS (
        ...
        -- Вставка данных в well_timeline_marker
    INSERT OR IGNORE INTO well_timeline_marker (
        wtm_wtl_id, wtm_focus, wtm_ed_uuid, wtm_mark_id, wtm_mark_data, wtm_restrict_mark
    )
    SELECT
        wtm_wtl_id, wtm_focus, wtm_ed_uuid, wtm_mark_id, wtm_mark_data, wtm_restrict_mark
    FROM source.well_timeline_marker
    WHERE NOT EXISTS (
        SELECT 1 FROM well_timeline_marker
        WHERE well_timeline_marker.wtm_wtl_id = source.well_timeline_marker.wtm_wtl_id
          AND well_timeline_marker.wtm_focus = source.well_timeline_marker.wtm_focus
          AND well_timeline_marker.wtm_ed_uuid = source.well_timeline_marker.wtm_ed_uuid
          AND well_timeline_marker.wtm_mark_id = source.well_timeline_marker.wtm_mark_id
    );
